...
if __name__=="__main__":
    ...
    # Reflected image (after alignment/mapping) - read from temp folder
    temp_dir = os.path.join(fn, f"{file_name}_temp")
    im_rl = cv2.imread(os.path.join(temp_dir, file_name+'+stack_0_hw_crop_mapped.png'))
    ...
    # Intermediate files go to temp_dir, final output goes to main directory
    save_fn = fn+file_name
    multi_svd.save_comp(save_fn, im_rl, im_tl, temp_dir=temp_dir)

    # Make sure the background is black
    im_target=cv2.imread(save_fn+"_hw_1.png")
    multi_svd.dark_bkg(save_fn,im_rl,im_target)
