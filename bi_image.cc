...
bi_image::bi_image(const char* tfile,int q_) : q(q_), dof(2*q*q), ftype(0),
	...
	f=(uint32 *) _TIFFmalloc(npixels *sizeof(uint32));

	if(TIFFReadRGBAImage(tif, width, height, f, 0)){
		puts("read img");
	}
	else{
...
bi_image::bi_image(int m_,int n_,int q_) : q(q_), dof(2*q*q), ftype(0), m(m_),
	n(n_), mn(m*n), pin(false), f(new double[mn]), g(NULL), of(NULL),
	al(new double[dof]) {
	setup_common();
}
...
bi_image::bi_image(bi_image &b) : q(b.q), dof(b.dof), ftype(b.ftype), m(b.m),
	...
	ay(b.ay), f(new double[mn]), g(NULL), of(NULL), al(new double[dof]),
	ilo(b.ilo), ihi(b.ihi), jlo(b.jlo), jhi(b.jhi) {
	memcpy(f,b.f,mn*sizeof(double));
	setup_common(false);
}
/** Constructs the image fitting class by copying the quantities from another
...
bi_image::bi_image(bi_image &b,int d) : q(b.q), dof(b.dof), ftype(b.ftype),
	...
	f(new double[mn]), g(NULL), of(NULL), al(new double[dof]),
...
void bi_image::smooth(double s) {
	if(g==NULL) g=new double[mn];

	// Use the g array to create a smoothed version of the f array, using a
	// five-point stencil
#pragma omp parallel for
	for(int j=0;j<n;j++) {
		double *fp=f+j*m,*fs=fp,*fe=fp+m,*fpen=fe-1,*gp=g+j*m;
		while(fp<fe) {
			double s1=0;int s2=0;
			if(j>0) {s1+=fp[-m];s2++;}
			if(j<n-1) {s1+=fp[m];s2++;}
			if(fp>fs) {s1+=fp[-1];s2++;}
...
#pragma omp for
		for(int j=0;j<n;j++) {
			double *fp=b.f+j*m,x,y=ay+j*dy,ylo=y-sth,yhi=y+sth;
		...
		for(int i=0;i<m;i++,fp++) {
			...
			*fp=0;
			for(double *zp=z;zp<z+2*o;) {
				delx=x-*(zp++);
				dely=y-*(zp++);
				*fp+=exp(-100*(delx*delx+dely*dely));
			}
		}
	}
}
...
inline double bi_image::gfunc(int ft,double a,double b) {
	switch(ft) {
		case 0: return (b-a)*(b-a);
		case 1: return a*(a-b);
		default: return 0;
	}
}
 ...
 * \return The function evaluation. */
inline double bi_image::gbfunc(int ft,double a,double b,double &gb) {
	switch(ft) {
		case 0:
			{
				double d=b-a;
 ...
 *	      grid. */
void bi_image::output_gnuplot(const char *filename,bool coords,bool primary) {
	...
	for(j=0;j<n;j++) {
		...
		while(bp<be) *(bp++)=static_cast<float>(*(fp++));
		fwrite(buf,sizeof(float),l,outf);
	}
